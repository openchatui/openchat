...
import { moveFolderSubmitAction } from "@/actions/files"
...
export function MoveFolderDialog({ open, onOpenChange, folderId }: MoveFolderDialogProps) {
  ...
  async function load(parent: string) {
    ...
    try {
      ...
      setFolders((data.folders as any[])?.map(f => ({ id: f.id, name: f.name })) ?? [])
    } catch (e: any) {
