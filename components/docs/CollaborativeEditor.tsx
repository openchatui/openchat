...
export function CollaborativeEditor({ documentId, user, token, className, chrome = 'card', initialHTML, enableGoogleSave, syncWithGoogle }: CollaborativeEditorProps) {
  ...
  useEffect(() => {
    ...
    const onUpdate = () => {
      ...
      pushTimer = setTimeout(async () => {
        try {
          ...
          const res = await fetch(`/api/drive/file/sync/${encodeURIComponent(documentId)}`, {
            method: 'POST',
    ...
    const poll = async () => {
      if (cancelled) return
      try {
        const meta = await fetch(`/api/drive/file/sync/${encodeURIComponent(documentId)}?mode=meta`).then(r => r.json()).catch(() => null) as any
        const modifiedMs = meta && typeof meta.modifiedMs === 'number' ? meta.modifiedMs : null
        if (modifiedMs && modifiedMs > lastPushMs + 1500) {
          const html = await fetch(`/api/drive/file/sync/${encodeURIComponent(documentId)}?mode=html`).then(r => r.text()).catch(() => '')
