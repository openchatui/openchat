import 'server-only';
import * as ConnectionsRepo from '@/lib/db/connections';
import * as ModelsRepo from '@/lib/db/models';
...
export class ProviderService {
  ...
  static async getConnectionForProvider(provider: SupportedProvider): Promise<ProviderConnection | null> {
    try {
      ...
      const connection = await ConnectionsRepo.getProviderConnection(provider);
  ...
  static async resolveAiProvider(input: ProviderResolutionInput): Promise<ProviderResolutionResult> {
    ...
    const modelRow = (await ModelsRepo.findModelById(requestedModel)) as any as ({ id: string; meta: unknown } | null);
